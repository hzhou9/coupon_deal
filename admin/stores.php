...
<ul>';                                                          
if( ab_to( array( 'coupons' => 'add' ) ) ) echo '<li><a href="?route=coupons.php&amp;action=add&amp;store=' . $_GET['id'] . '">' . $LANG['coupons_add_button'] . '</a></li>';
if( ab_to( array( 'products' => 'add' ) ) ) echo '<li><a href="?route=products.php&amp;action=add&amp;store=' . $_GET['id'] . '">' . $LANG['products_add_button'] . '</a></li>';
...
<div class="row"><span>' . $LANG['products'] . ':</span> <div>' . ( ab_to( array( 'products' => 'view' ) ) ? '<a href="?route=products.php&amp;action=list&amp;store=' . $info->ID . '">' . $info->products . '</a>' : $info->products ) . ( ab_to( array( 'products' => 'add' ) ) ? ' / <a href="?route=products.php&amp;action=add&amp;store=' . $info->ID . '&amp;category=' . $info->catID . '">' . $LANG['products_add_button'] . '</a>' : '' ) . '</div></div>
...
foreach( \query\main::while_stores( array_merge( array( 'orderby' => (isset( $_GET['orderby'] ) ? urldecode( $_GET['orderby'] ) : 'date desc') ), $options ) ) as $item ) {
  ...
  if( $ab_view_co ) {
    echo ( empty( $item->coupons ) ? $LANG['no_coupons_store'] : '<a href="?route=coupons.php&amp;store=' . $item->ID . '">' . sprintf( $LANG['nr_coupons_store'], $item->coupons ) . '</a>' );
    if( $ab_view_pr ) {
      echo '<br />';
    }
  }
  if( $ab_view_pr ) {
  echo ( empty( $item->products ) ? $LANG['no_products_store'] : '<a href="?route=products.php&amp;store=' . $item->ID . '">' . sprintf( $LANG['nr_products_store'], $item->products ) . '</a>' );
  }
  echo '</div>
  ...
  if( $ab_add_co ) echo '<a href="?route=coupons.php&amp;action=add&amp;store=' . $item->ID . '&amp;category=' . $item->catID . '">' . $LANG['coupons_add_button'] . '</a>';
  if( $ab_add_pr ) echo '<a href="?route=products.php&amp;action=add&amp;store=' . $item->ID . '&amp;category=' . $item->catID . '">' . $LANG['products_add_button'] . '</a>';
