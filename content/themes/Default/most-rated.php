...
$pagination = have_stores_custom( array( 'firstchar' => ( isset( $_GET['firstchar'] ) ? $_GET['firstchar'] : '' ), 'orderby' => 'votes DESC' ) );

echo '<ul class="letters">';
foreach( array_merge(range('A', 'Z'), array('0-9')) as $l ) {
  echo '<li' . ( isset( $_GET['firstchar'] ) && $_GET['firstchar'] == $l ? ' class="active"' : '' ) . '><a href="' . tlink( 'tpage/most-rated', 'firstchar=' . $l ) . '">' . $l . '</a></li>';
}
echo '<li><a href="' . tlink( 'tpage/most-rated' ) . '">ALL</a></li>';
echo '</ul>';
...
foreach( stores_custom( array( 'firstchar' => ( isset( $_GET['firstchar'] ) ? $_GET['firstchar'] : '' ), 'orderby' => 'votes DESC' ) ) as $item ) {
