<?php 
	...
	//--------------------------------------------------------------//
	    ...
	    if ($mysqli->connect_error) {
	        fail(_('Can\'t connect to database'));
	    }
...
<?php
	...
	if($feedback!=_('Some fields are missing.') && $feedback!=_('Email address is invalid.'))
	{
		...
		if ($r && mysqli_num_rows($r) > 0)
		{
					...
					if(str_replace(' ', '', $cf_fields_array[0])==$key && ($key!='name' && $key!='email' && $key!='list' && $key!='submit'))
					{
						...
						else
						{
							...
							//else if custom field format is Text
                                ...
                                $cf_value .= get_magic_quotes_gpc()?$value:mysqli_real_escape_string($mysqli, $value);
						}
					}
					else
					{
            ...
            if(isset($_POST['_update_custom_fields'])){
                $q = 'UPDATE subscribers SET custom_fields = "'.substr($cf_value, 0, -3).'" WHERE email = "'.$email.'" AND list = '.$list_id;
            }
			else if($opt_in)
			{
				if(!isset($_POST['name']))
			...
			if ($r)
			{
				...
				if(!$already_subscribed)
				{
					if($opt_in && $confirmed!=1)
						$feedback = _('Thank you, a confirmation email has been sent to you.');
					else
				...
				else
				{
					...
					{
						if($confirmed==0)
							$feedback = _('A confirmation email has already been sent to you.');
						else
